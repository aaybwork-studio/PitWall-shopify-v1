...
export function DriverLandingPage({ products, title, heroImage, bio, teamName, teamLink }: DriverLandingPageProps) {
  ...
  const hasTeamLink = !!(teamName && teamLink);
      ...
      >
        ...
        <div className="relative z-10 px-5 md:px-[100px] pb-6 w-full">
          ...
          <div className="font-mono text-[10px] uppercase tracking-widest mb-2" style={{ color: '#EDEBE5', opacity: 0.8 }}>
            ...
            {' / '}
            <a href="/collections" style={{ color: '#EDEBE5' }}>Drivers</a>
            {' / '}
            <span>{title}</span>
          </div>
        ...
        {hasTeamLink ? (
          <p className="font-mono text-sm leading-relaxed max-w-2xl" style={{ color: 'var(--fg)', opacity: 0.85 }}>
            ...
            <a
              ...
              style={{ color: 'var(--fg)', opacity: 1, textDecoration: 'underline', textUnderlineOffset: 3 }}
            >
              {teamName} →
            </a>
          </p>
